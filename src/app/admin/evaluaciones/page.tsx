...
import { Plus, Search, Edit, Eye, FileText, Trash2, Loader2 } from "lucide-react"
...
export default function EvaluationsPage() {
  ...
  const filtered = evaluations.filter((e) => {
    const matchSearch = e.student_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      (e.psychologist_name && e.psychologist_name.toLowerCase().includes(searchTerm.toLowerCase()))
        ...
        <CardContent>
          <div className="mb-4 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar por estudiante, test, psicólogo..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" />
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-muted/50">
                      ...
                      <th className="h-10 px-4 text-left font-medium">Psicólogo</th>
                  ...
                  <tbody>
                    {filtered.map((ev) => (
                      <tr key={ev.id} className="border-b last:border-0 hover:bg-muted/30">
                        ...
                        <td className="p-4 text-sm">{ev.psychologist_name || "-"}</td>
              ...
              <div className="md:hidden space-y-3">
                {filtered.map((ev) => (
                  <div key={ev.id} className="rounded-lg border p-4 space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{ev.test_name} • {ev.psychologist_name || "Sin psicólogo"}</p>
                  </div>
                ))}
              </div>
            </>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.observations} onChange={(e) => setForm({ ...form, observations: e.target.value })} placeholder="Notas" />
            </div>
            <div className="space-y-1.5">
